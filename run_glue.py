...
from peft import LoraConfig, PromptLearningConfig, get_peft_model
...
class TrainingArguments(transformers.TrainingArguments):
    adapter_name: str = field(
        default=None,
        metadata={"help": "Adapter to be used. Either 'lora' or 'polar'."},
    )
    regularization_lambda: float = field(
        default=1e-4, metadata={"help": "Regularization strength."}
    )
    init_lora_weights: str = field(
        default=True,
        metadata={
            "help": "Initialization strategy for LoRA: 'default', 'pissa', 'random''"
        },
    )
    gradient_type: str = field(
        default="landing",
        metadata={
            "help": "Gradient manipulation chosen in case adapter_name is 'polar'. Can be 'landing' or 'euclidean'."
        },
    )
    parametrize_S: str = field(
        default="identity",
        metadata={"help": "Parametrization on S. Can be 'diagonal', 'identity'."},
    )
...
def main():
    ...
    ### Added code
    if training_args.init_lora_weights == "default":
        init_strategy = True
    elif training_args.init_lora_weights == "random":
        init_strategy = False
    elif training_args.init_lora_weights == "pissa":
        init_strategy = "pissa"
    elif training_args.init_lora_weights == "symmetric_gaussian":
        init_strategy = "symmetric_gaussian"
    else:
        raise ValueError(
            f"Unknown init strategy for LoRA {training_args.init_lora_weights}"
        )
    peft_config = LoraConfig(
        ...
        parameterize_S=training_args.parametrize_S,
    ...
    if training_args.adapter_name == "polar":
        custom_module_mapping = {
            nn.Linear: partial(
                PoLARLinear, parameterization_lora_S=training_args.parametrize_S
            )
        }
    ...
    # just saving some duplicate args to make post-processing easier
    training_args_dict["dataset"] = data_args.task_name
    training_args_dict["rank"] = model_args.lora_rank
    training_args_dict["adapter"] = training_args.adapter_name
    training_args_dict["reg_lambda"] = training_args.regularization_lambda
    training_args_dict["lr"] = training_args.learning_rate
    with open(os.path.join(training_args.output_dir, "model_args.json"), "w") as file:
        json.dump(model_args_dict, file, indent=4)
    with open(
        os.path.join(training_args.output_dir, "training_args.json"), "w"
    ) as file:
        json.dump(training_args_dict, file, indent=4)
    ...
    callbacks = [
        StableRankWandbCallback(),
        OrthogonalityWandbCallback(rank=model_args.lora_rank),
    ]
    if training_args.adapter_name == "polar":
        callbacks += [
            LandingProjectionCallback(
                lambda_regul=training_args.regularization_lambda,
                gradient_type=training_args.gradient_type,
            )
        ]
    trainer = Trainer(
        model=model,
